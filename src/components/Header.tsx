...
type HeaderProps = {
  activePage?: 'home' | 'recipes' | 'meal-plans' | 'shopping-lists' | 'pantry' | 'family';
};
...
export default function Header({ activePage = 'home' }: HeaderProps) {
  ...
  const navLinks = [
    ...
    { name: 'Meal Plans', href: '/meal-plans', active: activePage === 'meal-plans' },
    { name: 'Shopping Lists', href: '/shopping-lists', active: activePage === 'shopping-lists' },
          ...
          <ul className="flex space-x-6">
            {navLinks.map((link) => (
              <li key={link.name}>
                <Link
                  href={link.href}
                  className={`hover:underline ${link.active ? 'font-bold' : ''}`}
                >
                  {link.name}
                </Link>
              </li>
